...
# API URL and headers
    ...
    # Sidebar menu
    with st.sidebar:
        st.image('src/image.png', width=200)
    ...
    if select_menu == menu[0]:  # Home
        ...
        if st.button("🎧 Convert to Speech", disabled=convert_disabled, type='primary'):
            ...
            if status_code == 200:
                ...
                st.info(f"ℹ️ Serverdagi ortiqcha yuklanish sabab qaytadan urunib ko'ring: {status_code}")

    else:  # About
        st.image('src/mnu.png', caption="TTSUZ")
        st.markdown("""
            ### Generative AI (GenAI)
